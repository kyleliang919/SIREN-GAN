...
class Generator(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.positional_encoding = nn.DataParallel(nn.Sequential(
                                       nn.Linear(2,100),
                                       Siren()
                                       ))
...
class RELU_GAN_MODEL(object):
    def __init__(self, args):
        print("RELU GAN model initalization.")
        ...
        for epoch in range(self.epochs):
            ...
            for i, (images, _) in enumerate(train_loader):
                ...
                if generator_iter % 1000 == 0:
                    ...
                    if not os.path.exists('training_result_images/'+ self.dataset + '/relu_gan/'):
                        os.makedirs('training_result_images/' + self.dataset + '/relu_gan/')
                    ...
                    utils.save_image(grid, 'training_result_images/'+self.dataset+'/relu_gan/img_generatori_iter_{}.png'.format(str(generator_iter).zfill(3)))
    ...
    def evaluate(self, test_loader, D_model_path, G_model_path):
        ...
        grid = utils.make_grid(samples)
        print("Grid of 8x8 images saved to 'relu_gan_model_image.png'.")
        utils.save_image(grid, self.dataset + '_relu_gan_model_image.png')
    ...
    def save_model(self):
        if not os.path.exists('./checkpoints/'+self.dataset + '/relu_gan/'):
            os.makedirs('./checkpoints/'+self.dataset + '/relu_gan/')
        torch.save(self.G.state_dict(), './checkpoints/'+self.dataset + '/relu_gan/generator.pkl')
        torch.save(self.D.state_dict(), './checkpoints/'+ self.dataset + '/relu_gan/discriminator.pkl')
        print('Models save to ./checkpoints/' + self.dataset + '/relu_gan/generator.pkl & ./checkpoints/' + self.dataset + '/relu_gan/discriminator.pkl ')
    ...
    def generate_latent_walk(self, number):
        if not os.path.exists('interpolated_images/'+self.dataset + '/relu_gan/'):
            os.makedirs('interpolated_images/'+self.dataset + '/relu_gan/')
        ...
        utils.save_image(grid, 'interpolated_images/'+self.dataset+'/relu_gan/interpolated_{}.png'.format(str(number).zfill(3)))
        print("Saved interpolated images to interpolated_images/"+ self.dataset + "/relu_gan/interpolated_{}.".format(str(number).zfill(3)))
