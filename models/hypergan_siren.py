...
class Hypernet(torch.nn.Module):
    def __init__(self, in_dims, out_dims):
        super().__init__()
        hidden_dims = reduce(mul,out_dims)
...
class Generator(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.img_dim = 32
        self.w_hypernets = [nn.DataParallel(Hypernet(64, [64,2])).cuda(), nn.DataParallel(Hypernet(64, [64,64])).cuda(), nn.DataParallel(Hypernet(64, [64,64])).cuda(),nn.DataParallel(Hypernet(64, [64,64])).cuda(), nn.DataParallel(Hypernet(64, [channels,64])).cuda()]
        self.b_hypernets = [nn.DataParallel(Hypernet(64, [64])).cuda(), nn.DataParallel(Hypernet(64, [64])).cuda(), nn.DataParallel(Hypernet(64, [64])).cuda(),nn.DataParallel( Hypernet(64, [64])).cuda(),nn.DataParallel( Hypernet(64, [channels])).cuda()]
        grid = np.zeros((2, self.img_dim * self.img_dim))
        for i in range(self.img_dim):
            for j in range(self.img_dim):
                grid[0][i * self.img_dim + j] = -1 + (2/self.img_dim) * i 
                grid[1][i * self.img_dim + j] = -1 + (2/self.img_dim) * j
        self.grid = torch.from_numpy(grid).float().cuda()
        self.siren = Siren()
        self.output = nn.Tanh()

    def forward(self, x):
        z = self.main_module(x)
        grid = self.grid
        for i in range(5):
            w = self.w_hypernets[i](z)
            b = self.b_hypernets[i](z)
            b = torch.repeat_interleave(b.unsqueeze(-1), self.grid.shape[-1], dim = -1)
            grid = self.siren(w @ grid + b)
        x = grid.view(-1, self.channels, self.img_dim, self.img_dim)
...
class HYPERGAN_SIREN_MODEL(object):
    def __init__(self, args):
        print("HYPERGAN SIREN model initalization.")
        ...
        for epoch in range(self.epochs):
            ...
            for i, (images, _) in enumerate(train_loader):
                ...
                if generator_iter % 1000 == 0:
                    ...
                    if not os.path.exists('training_result_images/'+ self.dataset + '/hypergan_siren/'):
                        os.makedirs('training_result_images/' + self.dataset + '/hypergan_siren/')
                    ...
                    utils.save_image(grid, 'training_result_images/'+self.dataset+'/hypergan_siren/img_generatori_iter_{}.png'.format(str(generator_iter).zfill(3)))
    ...
    def evaluate(self, test_loader, D_model_path, G_model_path):
        ...
        grid = utils.make_grid(samples)
        print("Grid of 8x8 images saved to 'hypergan_model_image.png'.")
        utils.save_image(grid, self.dataset + '_hypergan_siren_model_image.png')
    ...
    def save_model(self):
        if not os.path.exists('./checkpoints/'+self.dataset + '/hypergan_siren/'):
            os.makedirs('./checkpoints/'+self.dataset + '/hypergan_siren/')
        torch.save(self.G.state_dict(), './checkpoints/'+self.dataset + '/hypergan_siren/generator.pkl')
        torch.save(self.D.state_dict(), './checkpoints/'+ self.dataset + '/hypergan_siren/discriminator.pkl')
        print('Models save to ./checkpoints/' + self.dataset + '/hypergan_siren/generator.pkl & ./checkpoints/' + self.dataset + '/hypergan_siren/discriminator.pkl ')
    ...
    def generate_latent_walk(self, number):
        if not os.path.exists('interpolated_images/'+self.dataset + '/hypergan_siren/'):
            os.makedirs('interpolated_images/'+self.dataset + '/hypergan_siren/')
        ...
        utils.save_image(grid, 'interpolated_images/'+self.dataset+'/hypergan_siren/interpolated_{}.png'.format(str(number).zfill(3)))
        print("Saved interpolated images to interpolated_images/"+ self.dataset + "/hypergan_siren/interpolated_{}.".format(str(number).zfill(3)))
